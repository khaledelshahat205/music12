...
@Client.on_message(command(["playlist", f"playlist@VPllllllbot"]) & filters.group & ~filters.edited)
...
@Client.on_message(command(["player", f"player@VPllllllbot"]) & filters.group & ~filters.edited)
...
async def m_cb(b, cb):
    ...
    if type_ == "pause":
        ...
        if chet_id in callsmusic.pytgcalls.active_calls:
            ...
            callsmusic.pytgcalls.leave_group_call(chet_id)
            await cb.message.edit("⏹ **تم ايقاف التشغيل بنجاح..........!**")
        else:
            await cb.answer("الحســاب المساعــد  لم يتصــل بعــد!", show_alert=True)
...
async def play(_, message: Message):
    ...
    if message.chat.id in DISABLED_GROUPS:
        return    
    lel = await message.reply("**جار التحميل انتظر قليلاً....🤺🚨**")
            ...
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await USER.send_message(
                        message.chat.id, "انا الحساب المساعد لبوت تشغيل الاغاني 💞🤺"
                    )
                    await lel.edit(
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton("🔮𝐕𝘼𝙈𝘽𝙄𝙍🔮", url=f"https://t.me/XxlllllllllllllllllllllllllllxX")
                ],
            ]
        )
        ...
        # veez project
        try:
            toxxt = "قم باختيار النتيجه التي تريدها من الارقام بالاسفل 🚨💫:__\n\n"
            j = 0
            useer=user_name
            ...
            await lel.edit("جار التحميل انتظر قليلاً.....🤺🚨")
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        ...
        return
    #await cb.message.edit("**جار التحميل انتظر قليلاً...🤺🚨**")
    x=int(x)
    try:
